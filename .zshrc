...
bindkey "^P" up-line-or-beginning-search   # Up arrow: Search history
bindkey "^N" down-line-or-beginning-search # Down arrow: Search forward

# ────────────────────────────────────────────
# 🎨 UI & Productivity Enhancements
# ────────────────────────────────────────────
source ~/.config/zsh/plugins/fzf/shell/key-bindings.zsh  # FZF
source ~/.config/zsh/plugins/fzf-tab/fzf-tab.plugin.zsh  # FZF-Tab
eval "$(zoxide init zsh)"  # Zoxide for quick directory jumping
...
edit_and_execute_command() {
  ...
  if [[ $? -eq 0 ]]; then
    ...
    if [[ $? -eq 0 ]]; then
      [[ -n "$response" ]] && echo -e "\n\033[38;5;44m[OUTPUT]\033[0m\n\033[38;5;44m$response\033[0m"
    else
      [[ -n "$response" ]] && echo -e "\n\033[38;5;196m[ERROR]\033[0m\n\033[38;5;196m$response\033[0m"
    fi
